...
```{r}
heat <- read_csv("https://raw.githubusercontent.com/dwillis/jour405/refs/heads/main/data/pg_murder_temps.csv")
```
...
```{r}
heat |> summarize(correlation = cor(murders, temperature, method="pearson"))
```
...
There is a positive correlation between murders and temperatures but that doesn't mean that heat is causing more murder. There is a pretty wide line of best fit in the scatterplot that shows that even though there is a correlation, we aren't certain that this is what is causing it. 
