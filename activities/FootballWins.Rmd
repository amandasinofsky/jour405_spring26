...
This seems to be a pretty useful model. The high muliple and adjusted R-squared are a pretty good tell that the model accounts for a majority of the variability of wins. The very low p-value further confirms that the model is statistically significant. However, it isn't perfect because the residual standard error is a little high, but the model is overall useful. 
...
Points scored have the most impact on wins. Yards gained looks to be the least impactful but I wouldn't say that it seems to be not impactful at all. 
...
I could use this model to show the factors that are the most connected to winning games and how they can predict a win based on how many times they happen. 
