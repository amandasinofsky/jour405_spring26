...
The percent of people that have a bachelor's or higher is positively correlated to income. Percent unemployed is highly negatively correlated to voter turnout. Percent white is positively correlated to voter turnout. 

Percent unemployed has a pretty linear relationship with voter turnout. Income, median age, and bachelor's degree or higher also are pretty linear. None of them are extremely linear or compelling but there is a general linear relationship. Population is a lot more all over the place and there isn't really a clear linear relationship. 
...
There is a pretty small residual standard error. The multiple R-squared and adjusted R-squared are both really high which is a good sign. The p-value is also very low which means that this model is statistically significant. This seems to predict turnout very well and would be a useful model because of the parts of the model that determine fit and the coefficients indicate that it is a strong predictor as well. 
...
None of these models seem to be significantly better, but the best one out of the three is the pct_bachelors_higher, pct_white and income one. The adjusted R-squared is the highest out of the 3 and the closest to the original model. 
...
# Model 3
model3 <- lm(turnout ~ pct_bachelors_higher + pct_white + income, data = md_turnout)
summary(model3)
...
This analysis shows that factors like the percent of white people influences voter turnout positively, and percent unemployed influences in negatively. Other factors like income and population don't influence it as much. The percent of people who have a bachelor's or higher also doesn't really influence voter turnout. 
